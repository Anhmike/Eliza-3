...
class ElizaBot {
    /** @param {boolean} [noRandomFlag=false] */
    ...
    reset () {
        ...
        ElizaData.keywords.forEach( (a, k = 0) => { // might change to .map() if someone complains about IE6 compatibility or something
            
            let rules = ElizaData.keywords[k][2];
    ...
    init () {
      ...
      let synonymPatterns = {};
      if (ElizaData.synonyms) {
        
        for (let synonymIndex in ElizaData.synonyms) {
          
          let selectedSynonym = ElizaData.synonyms[synonymIndex].join('|');
          synonymPatterns[synonymIndex] = `(${synonymIndex}|${selectedSynonym})`;
      ...
      if (!ElizaData.keywords) {
        ElizaData.keywords = [['###', 0, [ ['###', []] ]]];
      }
      ...
      ElizaData.keywords.forEach( (a, k = 0) => {
        
        let rules = ElizaData.keywords[k][2];
        ElizaData.keywords[k][3] = k; // save original index for sorting
      ...
      ElizaData.keywords.sort(this._sortKeywords);
      // and compose regexps and refs for pres and posts
      ElizaBot.prototype.pres={};
      ElizaBot.prototype.posts={};
      if ((ElizaData.pres) && (ElizaData.pres.length)) {
        var a=new Array();
        for (var i=0; i<ElizaData.pres.length; i+=2) {
          a.push(ElizaData.pres[i]);
          ElizaBot.prototype.pres[ElizaData.pres[i]]=ElizaData.pres[i+1];
        }
      ...
      if ((ElizaData.posts) && (ElizaData.posts.length)) {
        var a=new Array();
        for (var i=0; i<ElizaData.posts.length; i+=2) {
          a.push(ElizaData.posts[i]);
          ElizaBot.prototype.posts[ElizaData.posts[i]]=ElizaData.posts[i+1];
        }
      ...
      // check for ElizaData.quits and install default if missing
      if ((!ElizaData.quits) || (typeof ElizaData.quits.length == 'undefined')) {
        ElizaData.quits=[];
      }
      // done
...
ElizaBot.prototype.transform = function(text) {
  ...
  for (var i=0; i<parts.length; i++) {
    ...
    if (part!='') {
      ...
      for (var q=0; q<ElizaData.quits.length; q++) {
        if (ElizaData.quits[q]==part) {
          this.quit=true;
      ...
      for (var k=0; k<ElizaData.keywords.length; k++) {
        if (part.search(new RegExp('\\b'+ElizaData.keywords[k][0]+'\\b', 'i'))>=0) {
...
ElizaBot.prototype._execRule = function(k) {
  ...
  for (var i=0; i<decomps.length; i++) {
    ...
    if (m!=null) {
      ...
      var rpl=reasmbs[ri];
      if (this.debug) alert('match:\nkey: '+ElizaData.keywords[k][0]+
        '\nrank: '+ElizaData.keywords[k][1]+
        '\ndecomp: '+decomps[i][0]+
        '\nreasmb: '+rpl+
        '\nmemflag: '+memflag);
...
ElizaBot.prototype._postTransform = function(s) {
  ...
  s=s.replace(/\s{2,}/g, ' ');
  s=s.replace(/\s+\./g, '.');
  if ((ElizaData.postTransforms) && (ElizaData.postTransforms.length)) {
    for (var i=0; i<ElizaData.postTransforms.length; i+=2) {
      s=s.replace(ElizaData.postTransforms[i], ElizaData.postTransforms[i+1]);
      ElizaData.postTransforms[i].lastIndex=0;
    }
  }
